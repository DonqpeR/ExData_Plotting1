...
res <- data[strptime(data$Date,"%d/%m/%Y")>="2007-02-01" & strptime(data$Date,"%d/%m/%Y")<"2007-02-03", c("Date","Time","Global_active_power")]
png(filename="plot2.png")
plot(strptime(paste(res$Date,res$Time),"%d/%m/%Y %H:%M"), res$Global_active_power, type="l", xlab="", ylab="Global Active Power (kilowatts)")
dev.off()
